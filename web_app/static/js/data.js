...
var Main = function () {
    ...
    _createClass(Main, [{
        ...
        value: function initialize() {
            ...
            // }
            this.drawInput();
            $('#output td').text('').removeClass('tr-success');
        }
    }, {
        key: 'onMouseDown',
        ...
        value: function drawInput() {
            ...
            var img = new Image();
            var img01 = this.canvas.toDataURL();
            img.onload = function () {
                var inputs = [];
                var small = document.createElement('canvas').getContext('2d');
                small.drawImage(img, 0, 0, img.width, img.height, 0, 0, 28, 28);
                // 得到图像数据 28 * 28
                // 通过 getImageData() 得到画布上指定矩形的像素数据
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        'host' : 'from wangbo',
                        'img': encodeURIComponent(img01)
                    },
                    success: function success(data) {
                        // 得到后台发过来的results 两列结果
                        for (var _i = 0; _i < 2; _i++) {
                            ...
                            for (var _j2 = 0; _j2 < 10; _j2++) {
                                if (_j2 === max_index) {
                                    $('#output tr').eq(_j2 + 1).find('td').eq(_i).addClass('tr-success');
                                } else {
                                    $('#output tr').eq(_j2 + 1).find('td').eq(_i).removeClass('tr-success');
                                }
                            }
                        }
                    }
